...
#include "cstdlib"
#include "../../include/containers/containers_errors.h"
	...
	void*(*copy_func)(const void*)) 
{
	...
	*error_code = SUCCESSFULLY_COMPLETED;
	...
	_vector->_base = malloc(_vector->_size * _vector->_data_size);
...
{
	if (vector == NULL || vector->_base == NULL || vector->_count < 1)return NULL;
...
{
	if (vector == NULL || vector->_base == NULL || vector->_count < 1 || index > vector->_count - 1) return NULL;
...
{
	if (vector == NULL) return VECTOR_IS_NULL;
	else if (vector->_base == NULL)return VECTOR_IS_CLEAR;
	else return vector->_size;
}
...
int vector_assign(vector * left, vector * right)
{
	...
	left->_base = malloc(left->_size * left->_data_size);
	if (left->_base == NULL)
...
int vector_insert(vector * vector, unsigned int position, void * data)
{
	...
	else if (position > vector->_count)return VECTOR_HIGH_POSSITION_IN_INSERT;
	...
	*/
	...
	void* temp1, *temp2;
	vector->_assigner(temp1, vector_at(vector, position));
	for (int i = position; i < vector->_count; i++) {
		temp2 = vector->_base[i + 1];
		vector->_base[i + 1] = temp1;
		temp1 = temp2;
	}
	vector->_count++;
	vector->_assigner(vector_at(vector,position), data);
...
int vector_erase(vector * vector, unsigned int position)
{
	...
	else if (vector->_count < 1)return VECTOR_IS_EMPTY;
	else if (vector->_count <= position)return VECTOR_ERASE_POSITION_OUT_OF_RANGE;
	
	for (int i = position; i < vector->_count - 1; i++)
...
{
	if (vector == NULL || vector->_base == NULL || vector->_count < 1)return true;
	else  false;
}
